...
let some_function_param = make_ident("number");
let some_function_procedure = make_procedure([make_declaration(some_function_param, null, Types.int)], Types.int, some_function_block);
let some_function_declaration = make_declaration(make_ident("some_function"), some_function_procedure);

let some_function_mul = make_binary_operation(some_function_param, "*", some_function_param);
let some_function_sub = make_binary_operation(some_function_mul, "-", make_literal("6"));
...
*/
let other_func_block = make_block();
let other_func_param = make_declaration(make_ident("number"), null, Types.int);
let other_func_procedure = make_procedure([other_func_param], Types.int, other_func_block);
let other_func_declaration = make_declaration(make_ident("some_other_function"), other_func_procedure);

let if_block = make_block();
let if_condition = make_binary_operation(other_func_param.ident, "%", make_literal("2"));
if_block.statements.push(newline);
if_block.statements.push(make_statement(make_return(make_literal("5"))));

let else_block = make_block();
else_block.statements.push(newline);
else_block.statements.push(make_statement(make_return(make_literal("-3"))));

other_func_block.statements.push(newline);
other_func_block.statements.push(make_statement(make_if(if_condition, if_block)));
other_func_block.statements.push(newline);
other_func_block.statements.push(make_statement(make_else(else_block)));
...
int main() {
	...
	while (local_variable < 24) {
		local_variable += some_other_function(local_variable);
	}
...
let other_func_call = make_procedure_call(other_func_declaration, [local_variable.ident]);
let local_variable_opassign = make_opassign(local_variable.ident, "+", other_func_call);
...
let transform_call_stack = new Array();
let transform_idents = new Set();
let transform_block_stack = new Array();
...
function run(target) {
	...
	if (target.base.kind == Code_Kind.RETURN) {
		...
		while (!Object.is(expr, last_call)) {
...
function get_final_name(name) {
	...
	while (transform_idents.has(final_name)) {
	...
	transform_idents.add(final_name);
...
function transform(node) {
	...
	if (node.base.kind == Code_Kind.PROCEDURE_CALL) {
		...
		// @Audit
		transform_call_stack.push(node);

		node.map_arg_replace = new Map();
		...
		if (procedure.parameters) {
			...
			for (let param_index = 0; param_index < procedure.parameters.length; param_index += 1) {
				...
				node.map_arg_replace.set(param.ident, param_ident);
		...
		let last_call = transform_call_stack[transform_call_stack.length-1];
		...
		transform_block_stack.push(node);

		let trimming_newlines = true;
		...
		// @Copypaste
		let ident_replacement = last_call.map_arg_replace.get(node.declaration.ident);

		if (ident_replacement) {

			replacement.statements.push(make_statement(ident_replacement));
		}
	}
